...
- **Two theming systems coexist; new code must use `Theme.of(context)`, not `AppColors`.** The static `AppColors.*` getters read a global mutable `AppColors.isDarkMode` and do NOT register a dependency on the widget tree — a page that only reads `AppColors` will not rebuild when the OS switches light/dark, so it shows stale colors until it happens to rebuild for another reason (navigating to it fresh, a `Theme.of(context)` call elsewhere in its `build`, etc.). New pages/widgets should read colors and text styles from the context so they react live: semantic colors without a `ColorScheme` slot (`textDark`, `textMuted`, `primaryLight`, `skeletonBase`, the six `risk*`) come from `Theme.of(context).extension<AppColorsExt>()!` ([lib/core/theme/app_colors_ext.dart](lib/core/theme/app_colors_ext.dart)); `primary`/`background`/`cardBackground`/`error` come from `Theme.of(context).colorScheme`; font sizes come from `Theme.of(context).textTheme` (`titleLarge` 20, `bodyLarge` 16, `bodyMedium` 14, `bodySmall` 12) instead of hardcoded `fontSize:`. `AppColors` and `AppColorsExt` are fed from the SAME per-variant literals in `theme.dart`, so the two systems never drift. `AppColors` is legacy — leave existing pages as-is; only switch a widget to `Theme.of` when you're already rewriting it (a full migration of all ~39 pages is deliberately out of scope).
